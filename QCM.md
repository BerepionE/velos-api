...
- [ ] A. Parce que le rebase réécrit l'historique et change l'identifiant des commits, ce qui casse le travail de ceux qui les avaient déjà récupérés
- [ ] B. Parce que le rebase est plus lent que la fusion
- [x] C. Parce que le rebase supprime les branches distantes
...
- [ ] A. Le script d'initialisation n'est exécuté qu'à la **première** initialisation du stockage : un volume déjà existant fait considérer la base comme déjà installée
- [ ] B. Le script contient une erreur de syntaxe silencieuse
- [ ] C. Il faut publier le port de la base pour que le script s'exécute
- [x] D. Le script doit être copié dans l'image, il ne peut pas être monté
...
- [x] A. L'étiquette de l'image contient une majuscule
- [ ] B. Le nombre d'exemplaires demandé est trop élevé
- [ ] C. Le cluster tourne dans ses propres conteneurs et ne voit pas le stock d'images de ta machine : il faut l'y charger, ou la tirer d'un registre accessible
...
- [x] B. Parce que la variante interne contient davantage de droits
- [ ] C. Parce que le cluster refuse les connexions venant de l'extérieur du navigateur
- [ ] D. Parce que Jenkins tourne dans un conteneur : l'adresse de boucle locale y désigne Jenkins lui même et non le cluster, il faut donc l'adresse joignable depuis le réseau des conteneurs
...
- [ ] B. Un contrôle très étendu sur la machine hôte : qui peut piloter le moteur Docker peut, en pratique, obtenir les pleins pouvoirs sur cette machine
- [x] C. Un risque de saturation du disque uniquement
...
- [x] J'ai coché **une seule** case par question, pour les 40 questions
- [x] J'ai écrit mon nom et mon groupe en haut du fichier
- [x] J'ai renommé ce fichier en `QCM.md` et déposé à la racine de mon dépôt
